...
function AdminListingFormContent() {
    ...
    // Images State
    const [existingImages, setExistingImages] = useState([]); // Array of existing image URL strings
    const [images, setImages] = useState([]); // Array of new File objects
    const [previewUrls, setPreviewUrls] = useState([]);

    // UI state
    const [loading, setLoading] = useState(false);
    const [fetchingDetail, setFetchingDetail] = useState(isEditMode);
            ...
            getListingDetail(editId)
                .then(res => {
                    if (res.data?.success && res.data.listing) {
                        ...
                        const currentImgs = Array.isArray(l.images) ? l.images : [];
                        setExistingImages(currentImgs);
                        setPreviewUrls(currentImgs.map(img => getImageUrl(img)));
                    } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Attach newly added image files
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                ...
                                <span>{isEditMode ? 'Änderungen speichern' : 'Inserat veröffentlichen'}</span>
                            </>
                        )}
                    </button>
                </div>
                    ...
                    <div className="bg-white border border-[#E8EAEF] rounded-3xl p-6 sm:p-7 shadow-2xs space-y-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <p className="text-[10px] text-slate-400">
                                💡 <strong>Tipp:</strong> Wenn du bereits Stichpunkte oder Text im Feld hast, baut die Auto-Generierung deine Angaben direkt strukturiert in den Anzeigentext ein!
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="pt-3">
                            ...
                            >
                                {loading ? (
                                    <>
                                        <Loader2 className="w-4 h-4 animate-spin text-gold" />
                                        <span>{isEditMode ? 'Wird gespeichert...' : 'Wird veröffentlicht...'}</span>
                                    </>
                                ) : (
                                    <>
                                        {isEditMode ? (
                                            <Save className="w-4 h-4 text-gold stroke-[2.5]" />
                                        ) : (
                                            <Plus className="w-4 h-4 text-gold stroke-[3]" />
                                        )}
                                        <span>{isEditMode ? 'Änderungen speichern' : 'Inserat jetzt veröffentlichen'}</span>
                                    </>
                                )}
                            </button>
                        </div>
