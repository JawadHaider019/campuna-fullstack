...
const MENU_ITEMS = [
    ...
    { label: 'Inserate & Moderation', path: '/admin/inserate', icon: BarChart3 },
...
export default function AdminLayout({ children }) {
    ...
    return (
        <div className="h-screen w-full bg-gradient-to-br from-[#004709] via-[#002204] to-[#040805] text-slate-800 font-sans flex flex-col lg:flex-row p-2 sm:p-3 lg:p-4 gap-2 sm:gap-3 lg:gap-4 overflow-hidden">
            
            {/* ─── FIXED FOREST-TO-BLACK SIDEBAR ─── */}
            <aside className="hidden lg:flex flex-col w-[230px] xl:w-[250px] p-5 justify-between shrink-0 text-white relative bg-transparent h-full overflow-y-auto">
                
                <div className="space-y-8">
                    {/* Deep Sunken / Recessed Logo Cavity Container */}
                    <div className="relative rounded-2xl px-4 py-3 bg-gradient-to-b from-[#D5D9E0] via-[#ECEEF2] to-[#FFFFFF] shadow-[inset_0_5px_10px_rgba(0,0,0,0.38),inset_0_2px_4px_rgba(0,0,0,0.3),0_1px_1px_rgba(255,255,255,0.15)] border border-black/30 ring-1 ring-white/10 flex items-center justify-center overflow-hidden">
                        {/* Recessed vignette overlay */}
                        <div className="absolute inset-0 rounded-2xl pointer-events-none shadow-[inset_0_8px_16px_rgba(0,0,0,0.25)]" />
                        <Image
                            src="/logo.webp"
                            alt="Campuna"
                            width={140}
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between p-2.5 rounded-xl text-xs font-semibold ${
                                            isActive ? 'bg-gold text-forest font-bold' : 'text-sand/80'
                                        }`}
                                    >
            ...
            <div className="flex-1 bg-[#FFFFFF] rounded-[24px] sm:rounded-[32px] lg:rounded-[36px] overflow-hidden flex flex-col shadow-2xl h-full border border-white/10">
                <main className="p-5 sm:p-7 md:p-9 flex-1 overflow-y-auto w-full">
                    {children}
                </main>
            </div>
        </div>
